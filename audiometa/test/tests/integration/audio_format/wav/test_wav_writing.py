...
class TestWavWriting:
    def test_writing_default_format_wav(self):
        with temp_file_with_metadata({}, "wav") as test_file_path:
            ...
            title = RIFFMetadataGetter.get_title(test_file_path)
    ...
    def test_id3v2_3_metadata_writing_wav(self):
        with temp_file_with_metadata({}, "id3v2.3") as test_file_path:
            ...
            update_metadata(
                test_file_path, unified_metadata=metadata, metadata_format=MetadataFormat.ID3V2, id3v2_version=(2, 3, 0)
            )

            assert ID3v2HeaderVerifier.get_id3v2_version(test_file_path) == (2, 3, 0)

            title = ID3v2MetadataGetter.get_title(test_file_path)
