...
class TestWavReading:
    ...
    def test_riff_metadata_reading_wav(self):
        with temp_file_with_metadata({}, "wav") as test_file_path:
            RIFFMetadataSetter.set_title(test_file_path, "a" * 30)

            metadata = get_unified_metadata(test_file_path, metadata_format=MetadataFormat.RIFF)
            assert metadata.get(UnifiedMetadataKey.TITLE) == "a" * 30

    def test_vorbis_metadata_reading_wav(self):
        with temp_file_with_metadata({}, "wav") as test_file_path:
            with pytest.raises(MetadataFormatNotSupportedByAudioFormatError):
                get_unified_metadata(test_file_path, metadata_format=MetadataFormat.VORBIS)
