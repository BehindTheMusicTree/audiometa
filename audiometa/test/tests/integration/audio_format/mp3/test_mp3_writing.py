...
class TestMp3Writing:
    ...
    def test_id3v2_3_metadata_writing_mp3(self):
        with temp_file_with_metadata({}, "id3v2.3") as test_file_path:
            ...
            update_metadata(test_file_path, metadata, metadata_format=MetadataFormat.ID3V2, id3v2_version=(2, 3, 0))
            read_metadata = get_unified_metadata(test_file_path, metadata_format=MetadataFormat.ID3V2)
    ...
    def test_riff_metadata_writing_mp3(self):
        with temp_file_with_metadata({}, "mp3") as test_file_path:
            with pytest.raises(MetadataFormatNotSupportedByAudioFormatError):
                update_metadata(test_file_path, {UnifiedMetadataKey.TITLE: "Test Title RIFF"}, metadata_format=MetadataFormat.RIFF)
                
                
    def test_vorbis_metadata_writing_mp3(self):
        with temp_file_with_metadata({}, "mp3") as test_file_path:
            with pytest.raises(MetadataFormatNotSupportedByAudioFormatError):
                update_metadata(test_file_path, {UnifiedMetadataKey.TITLE: "Test Title Vorbis"}, metadata_format=MetadataFormat.VORBIS)