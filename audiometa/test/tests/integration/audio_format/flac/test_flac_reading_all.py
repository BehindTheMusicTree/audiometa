...
class TestFlacReading:
    def test_all_metadata_reading_flac(self):
        with temp_file_with_metadata({}, "flac") as test_file_path:
            VorbisMetadataSetter.add_title(test_file_path, "a" * 30)
    ...
    def test_vorbis_metadata_reading_flac(self):
        with temp_file_with_metadata({}, "flac") as test_file_path:
            ...
            metadata = get_unified_metadata(test_file_path, metadata_format=MetadataFormat.VORBIS)
            assert metadata.get(UnifiedMetadataKey.TITLE) == "a" * 30

    def test_id3v2_3_metadata_reading_flac(self):
        with temp_file_with_metadata({}, "flac") as test_file_path:
            ID3v2MetadataSetter.set_metadata(
                test_file_path, {"title": "ID3v2.3 Long Title That Exceeds ID3v1 Limits"}, version="2.3"
            )

            metadata = get_unified_metadata(
                test_file_path, metadata_format=MetadataFormat.ID3V2, id3v2_version=(2, 3, 0)
            )
    ...
    def test_id3v2_4_metadata_reading_flac(self):
        with temp_file_with_metadata({}, "flac") as test_file_path:
            ID3v2MetadataSetter.set_metadata(
                test_file_path, {"title": "ID3v2.4 Long Title That Exceeds ID3v1 Limits"}, version="2.4"
            )

            metadata = get_unified_metadata(
                test_file_path, metadata_format=MetadataFormat.ID3V2, id3v2_version=(2, 4, 0)
            )
    ...
    def test_id3v1_metadata_reading_flac(self):
        with temp_file_with_metadata({}, "flac") as test_file_path:
            ID3v1MetadataSetter.set_metadata(test_file_path, {"title": "a" * 30})
            assert ID3v1MetadataGetter.get_title(test_file_path) == "a" * 30

            metadata = get_unified_metadata(test_file_path, metadata_format=MetadataFormat.ID3V1)
            assert metadata.get(UnifiedMetadataKey.TITLE) == "a" * 30

    def test_riff_metadata_reading_flac(self):
        with temp_file_with_metadata({}, "flac") as test_file_path:
            with pytest.raises(MetadataFormatNotSupportedByAudioFormatError):
                get_unified_metadata(test_file_path, metadata_format=MetadataFormat.RIFF)
