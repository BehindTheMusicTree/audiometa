...
class TestArtistsDeleting:
    def test_delete_artists_id3v2(self):
        with temp_file_with_metadata({}, "mp3") as test_file_path:
            ID3v2MetadataSetter.set_artists(test_file_path, "Artist 1; Artist 2")
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.ARTISTS) == ["Artist 1", "Artist 2"]

            # Delete metadata using library API
            update_metadata(test_file_path, {UnifiedMetadataKey.ARTISTS: None}, metadata_format=MetadataFormat.ID3V2)
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.ARTISTS) is None

    def test_delete_artists_id3v1(self):
        with temp_file_with_metadata({}, "id3v1") as test_file_path:
            ID3v1MetadataSetter.set_artist(test_file_path, "Artist 1")
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.ARTISTS) == ["Artist 1"]

            # Delete metadata using library API
            update_metadata(test_file_path, {UnifiedMetadataKey.ARTISTS: None}, metadata_format=MetadataFormat.ID3V1)
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.ARTISTS) is None

    def test_delete_artists_riff(self):
        with temp_file_with_metadata({}, "wav") as test_file_path:
            RIFFMetadataSetter.set_artist(test_file_path, "Artist 1")
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.ARTISTS) == ["Artist 1"]

            # Delete metadata using library API
            update_metadata(test_file_path, {UnifiedMetadataKey.ARTISTS: None}, metadata_format=MetadataFormat.RIFF)
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.ARTISTS) is None

    def test_delete_artists_vorbis(self):
        with temp_file_with_metadata({}, "flac") as test_file_path:
            VorbisMetadataSetter.set_artists(test_file_path, ["Artist 1", "Artist 2"])
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.ARTISTS) == ["Artist 1", "Artist 2"]

            # Delete metadata using library API
            update_metadata(test_file_path, {UnifiedMetadataKey.ARTISTS: None}, metadata_format=MetadataFormat.VORBIS)
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.ARTISTS) is None
    ...
    def test_delete_artists_empty_list(self):
        with temp_file_with_metadata({}, "mp3") as test_file_path:
            ID3v2MetadataSetter.set_artists(test_file_path, "")
