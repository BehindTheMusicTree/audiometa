...
class TestAlbumArtistsDeleting:
    def test_delete_album_artists_id3v2(self):
        with temp_file_with_metadata({}, "mp3") as test_file_path:
            ...
            update_metadata(
                test_file_path, {UnifiedMetadataKey.ALBUM_ARTISTS: None}, metadata_format=MetadataFormat.ID3V2
            )
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.ALBUM_ARTISTS) is None
    ...
    def test_delete_album_artists_riff(self):
        with temp_file_with_metadata({}, "wav") as test_file_path:
            ...
            album_artists = get_unified_metadata_field(
                test_file_path, UnifiedMetadataKey.ALBUM_ARTISTS, metadata_format=MetadataFormat.RIFF
            )
            assert album_artists == ["Test Album Artist"]

            # Delete metadata by setting to None
            update_metadata(
                test_file_path, {UnifiedMetadataKey.ALBUM_ARTISTS: None}, metadata_format=MetadataFormat.RIFF
            )
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.ALBUM_ARTISTS) is None
    ...
    def test_delete_album_artists_vorbis(self):
        with temp_file_with_metadata({}, "flac") as test_file_path:
            ...
            # Delete metadata by setting to None
            update_metadata(
                test_file_path, {UnifiedMetadataKey.ALBUM_ARTISTS: None}, metadata_format=MetadataFormat.VORBIS
            )
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.ALBUM_ARTISTS) is None
