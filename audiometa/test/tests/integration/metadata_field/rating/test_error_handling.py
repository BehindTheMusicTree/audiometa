    ...
    def test_rating_negative_value_rejected_in_normalized_mode(self):
        with temp_file_with_metadata({"title": "Test Title", "artist": "Test Artist"}, "mp3") as test_file_path:
            with pytest.raises(InvalidRatingValueError) as exc_info:
                update_metadata(test_file_path, {UnifiedMetadataKey.RATING: -1}, normalized_rating_max_value=100)
            assert "must be non-negative" in str(exc_info.value)

    def test_rating_over_max_value_rejected_in_normalized_mode(self):
        with temp_file_with_metadata({"title": "Test Title", "artist": "Test Artist"}, "mp3") as test_file_path:
            with pytest.raises(InvalidRatingValueError) as exc_info:
                update_metadata(test_file_path, {UnifiedMetadataKey.RATING: 101}, normalized_rating_max_value=100)
