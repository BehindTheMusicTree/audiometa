...
class TestFieldNotSupportedReading:
    ...
    def test_id3v1(self):
        with temp_file_with_metadata({"title": "Test Song"}, "id3v1") as test_file, pytest.raises(
            MetadataFieldNotSupportedByLib, match="FIELD_NOT_SUPPORTED metadata not supported by the library."
        ):
            get_unified_metadata_field(test_file, "FIELD_NOT_SUPPORTED", metadata_format=MetadataFormat.ID3V1)

    def test_id3v2(self):
        with temp_file_with_metadata({"title": "Test Song"}, "mp3") as test_file, pytest.raises(
            MetadataFieldNotSupportedByLib, match="FIELD_NOT_SUPPORTED metadata not supported by the library."
        ):
            get_unified_metadata_field(test_file, "FIELD_NOT_SUPPORTED", metadata_format=MetadataFormat.ID3V2)

    def test_vorbis(self):
        with temp_file_with_metadata({"title": "Test Song"}, "flac") as test_file, pytest.raises(
            MetadataFieldNotSupportedByLib, match="FIELD_NOT_SUPPORTED metadata not supported by the library."
        ):
            get_unified_metadata_field(test_file, "FIELD_NOT_SUPPORTED", metadata_format=MetadataFormat.VORBIS)

    def test_riff(self):
        with temp_file_with_metadata({"title": "Test Song"}, "wav") as test_file, pytest.raises(
            MetadataFieldNotSupportedByLib, match="FIELD_NOT_SUPPORTED metadata not supported by the library."
        ):
