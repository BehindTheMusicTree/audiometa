...
class TestBpmDeleting:
    def test_delete_bpm_id3v2(self):
        with temp_file_with_metadata({}, "mp3") as test_file_path:
            ID3v2MetadataSetter.set_bpm(test_file_path, 120)
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.BPM) == 120

            # Delete metadata using library API
            update_metadata(test_file_path, {UnifiedMetadataKey.BPM: None}, metadata_format=MetadataFormat.ID3V2)
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.BPM) is None
    ...
    def test_delete_bpm_id3v1(self):
        ...
        with temp_file_with_metadata({}, "mp3") as test_file_path:
            with pytest.raises(MetadataFieldNotSupportedByMetadataFormatError):
                update_metadata(test_file_path, {UnifiedMetadataKey.BPM: None}, metadata_format=MetadataFormat.ID3V1)

    def test_delete_bpm_riff(self):
        with temp_file_with_metadata({"bpm": 120}, "wav") as test_file_path:
            raw_metadata = RIFFMetadataGetter.get_raw_metadata(test_file_path)
            assert "ibpm=120" in raw_metadata

            update_metadata(test_file_path, {UnifiedMetadataKey.BPM: None}, metadata_format=MetadataFormat.RIFF)
            raw_metadata_after = RIFFMetadataGetter.get_raw_metadata(test_file_path)
    ...
    def test_delete_bpm_vorbis(self):
        with temp_file_with_metadata({}, "flac") as test_file_path:
            VorbisMetadataSetter.set_bpm(test_file_path, 120)
            assert (
                get_unified_metadata_field(
                    test_file_path, UnifiedMetadataKey.BPM, metadata_format=MetadataFormat.VORBIS
                )
                == 120
            )

            update_metadata(test_file_path, {UnifiedMetadataKey.BPM: None}, metadata_format=MetadataFormat.VORBIS)
            assert get_unified_metadata_field(test_file_path, UnifiedMetadataKey.BPM) is None
    ...
    def test_delete_bpm_zero(self):
        with temp_file_with_metadata({}, "mp3") as test_file_path:
            ID3v2MetadataSetter.set_bpm(test_file_path, 0)
