...
class TestCLIHelp:
    ...
    def test_cli_unified_help(self):
        result = subprocess.run(
            [sys.executable, "-m", "audiometa", "unified", "--help"], capture_output=True, text=True
        )
        assert result.returncode == 0
        assert "unified" in result.stdout.lower()

    def test_cli_write_help(self):
        result = subprocess.run([sys.executable, "-m", "audiometa", "write", "--help"], capture_output=True, text=True)
        assert result.returncode == 0
        assert "write" in result.stdout.lower()

    def test_cli_delete_help(self):
        result = subprocess.run([sys.executable, "-m", "audiometa", "delete", "--help"], capture_output=True, text=True)
        assert result.returncode == 0
